...
int average_waiting_time=0,average_turnaround=0;    
int process_count=0;

struct proc* queue1 = 0;
struct proc* queue2 = 0;
struct proc* queue3 = 0;
...
void Queue2(int pid,int burst,int prior)
{
    ...
    struct proc* p=queue2;
        if(queue2->priority>prior)
        {
            ...
            while(p->next!=0&&p->next->priority<prior)
            {
                p=p->next;
            }
            temp->next=p->next;
...
int process_exhausted(struct proc* Queue)
{
    struct proc *p=Queue;
    if(isEmpty(Queue))
        return 1;
...
void scheduling_priority()
{
   int outer_time=10;
   if(isEmpty(queue2))
        return;
    struct proc* ptr=queue2;
    int total_time=10;
    while(total_time>0)
    {
